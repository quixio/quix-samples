...
namespace QuixTracker.Droid
{
	...
	public class PeriodicBackgroundService : Android.App.Service, ISensorEventListener
	{
		...
		{
			FlurlHttp.ConfigureClient("https://writer-ziadmoussa1-workspace1.platform.quix.ai", cli => cli.Settings.HttpClientFactory = new UntrustedCertClientFactory());
		...
		async Task StartGeoLocationTracking()
		{
            ...
            //		  .SetInterval(MainPage.Instance.Interval)
            //		  .SetFastestInterval(MainPage.Instance.Interval)
            //		  .SetMaxWaitTime(MainPage.Instance.Interval*10);

            //var callback = new LocationCallbackExt(this);
