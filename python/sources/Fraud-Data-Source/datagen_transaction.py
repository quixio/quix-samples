...
class Customer:
    ...
    def print_trans(self, trans, is_fraud, fraud_dates, use_transaction_date_as_timestamp):
        ...
        for t in trans[0]:
            ...
            if (is_fraud == 0 and t[1] not in fraud_dates) or is_fraud == 1:
                ...
                if use_transaction_date_as_timestamp: 
                    ...
                    timestamp = datetime.strptime(temp_value, "%Y-%m-%d %H:%M:%S")
                else:
                    timestamp = datetime.utcnow()

                ts = data.add_timestamp(timestamp)
                for i, x in enumerate(all_data):
                    ts.add_value(all_headers[i], x)
...
def valid_int(s):
    try:
        ...
        msg = "not a valid integer: {0!r}".format(s)
        raise argparse.ArgumentTypeError(msg)

def valid_boolean(s):
    neg_values = ['no', 'false', '0', 'n', 'f', 'nope']
    pos_values = ['true', '1', 't', 'y', 'yes', 'yeah']
    try:
        print(s)
        if s is None: return False
        # is the value supplied valid?
        if s.lower() not in neg_values and s.lower() not in pos_values: 
            raise ValueError
        return s.lower() in pos_values
    except ValueError:
        msg = "not a valid boolean value: {0!r}. Use true, 1, yeah, yes, t or y for Truthy values and no, false, 0, n, f, nope for Falsy values ".format(s)
        raise argparse.ArgumentTypeError(msg)
...
def main(customer_file, profile_file, start_date, end_date, out_path=None, start_offset=0, end_offset=sys.maxsize,
    ...
    with open(customer_file, 'r') as f:
        ...
        # [sr] these are the headers
        joined_headers = headers + transaction_headers
            ...
            for row in f.readlines():
                ...
                if cust.attrs['profile'] == profile_name:
                    ...
                    if fraud_flag < 99: #11->25
                        ...
                        cust.print_trans(temp_tx_data, is_fraud, fraud_dates, use_transaction_date_as_timestamp)
