...
###########################################
stream_producer.properties.name = "JSON Demo Data"  # Give the stream a human-readable name (for the data catalogue).
...
def process_json_file(csv_file):
    ...
    # fill any None/null vales with empty string.
    df = df.fillna(value="")
...
processing_thread = threading.Thread(target=process_json_file, args=("demo-data.json",))
